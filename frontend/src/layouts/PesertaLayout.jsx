// src/layouts/PesertaLayout.jsx (VERSI FINAL - DENGAN NOTIFIKASI CONTEXT)
...
function PesertaLayout() {
  ...
  // 🔥🔥🔥 DISABLE SCROLL BODY SAAT MODAL FULLSCREEN TERBUKA
  useEffect(() => {
    if (isModalOpen) {
  ...
  const isPenilaianActive = () => {
    return location.pathname === "/peserta/laporan-akhir" || 
           location.pathname === "/peserta/nilai-akhir"
  }

  // Breadcrumb - Hilangkan ID dari URL
  const getPageTitle = () => {
    const path = location.pathname;

    // Jika di halaman detail tugas (kumpul)
    if (path.includes("/kumpul")) {
      return "Tugas / Kumpul";
    }

    // Jika di halaman detail tugas (lihat)
    if (path.startsWith("/peserta/tugas/") && path.match(/\/\d+$/) && !path.includes("/kumpul")) {
      return "Detail Tugas";
    }

    const titles = {
      "/peserta/dashboard": "Dashboard",
      "/peserta/presensi": "Check-in / Check-out",
      "/peserta/riwayat-presensi": "Riwayat Presensi",
      "/peserta/materi-mentor": "Materi Mentor",
      "/peserta/tugas": "Daftar Tugas",
      "/peserta/materi-kompetensi": "Materi Kompetensi",
      "/peserta/daftar-kuis-kompetensi": "Kuis Kompetensi",
      "/peserta/sertifikat": "Sertifikat",
      "/peserta/laporan-akhir": "Laporan Akhir",
      "/peserta/nilai-akhir": "Nilai Akhir",
      "/peserta/profile": "Profil Saya"
    };
  ...
  const SidebarContent = ({ collapsed: isCollapsed, onMenuClick }) => (
    <div className="h-full flex flex-col">
      ...
      {/* MENU NAVIGATION - scroll hanya jika menu melebihi tinggi */}
        ...
        <ul className="space-y-1 text-sm">
          ...
          <li>
            ...
            >
              <div className="flex items-center gap-3">
                <Award size={18} />
                {!isCollapsed && <span className="font-medium">Penilaian Akhir</span>}
              </div>
              {!isCollapsed && (
            ...
            {!isCollapsed && penilaianOpen && (
              <div className="ml-7 mt-2 space-y-1">
                <Link to="/peserta/laporan-akhir" onClick={onMenuClick}>
                  <div className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all duration-200 cursor-pointer ${
                    location.pathname === "/peserta/laporan-akhir"
                      ? "bg-teal-50 text-teal-600 font-medium"
                      : "text-gray-500 hover:bg-gray-100"
                  }`}>
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden relative">
      ...
      <div className="flex flex-1 overflow-hidden pt-1">
        ...
        <div className="flex-1 flex flex-col bg-white relative z-10 h-full overflow-hidden">
          ...
          {/* PAGE CONTENT - TETAP MUNCUL SEBAGAI BACKGROUND MODAL */}
      ...
      {/* LOGOUT CONFIRMATION MODAL - z-index 9999 */}
      {logoutConfirmOpen && (
        <div className="fixed inset-0 z-[9999] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 py-10 animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden animate-zoomIn max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-5 border-b border-gray-200 bg-gradient-to-r from-red-50 to-orange-50 sticky top-0">
              <div className="flex items-center gap-3">
