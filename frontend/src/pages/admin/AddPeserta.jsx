...
function AddPeserta() {
  ...
  const isFormComplete = () => {
    return (
      ...
      isNameValid() && isPhoneValid() && isDivisiValid() &&
      isMentorValid() && isKampusValid() && isProdiValid() &&
      isTanggalMulaiValid() && isTanggalSelesaiValid()
    )
  }
  ...
  const getFieldError = (field) => {
    ...
    switch(field) {
      ...
      case 'tanggal_selesai':
        ...
        if (!isTanggalSelesaiValid()) return "Tanggal selesai tidak boleh sebelum tanggal mulai"
        return null
      default:
        return null
    }
  ...
  const validateForm = () => {
    setTouched({
      email: true, password: true, password_confirmation: true,
      nama: true, no_telepon: true, id_divisi: true, id_mentor: true,
      asal_kampus: true, prodi: true, tanggal_mulai: true, tanggal_selesai: true,
    })

    if (!isEmailValid()) { setError("Email tidak valid"); return false; }
    if (!isPasswordValid()) { setError("Password tidak valid"); return false; }
    ...
    if (!isPhoneValid()) { setError("Nomor telepon tidak valid"); return false; }
    ...
    if (!isTanggalSelesaiValid()) { setError("Tanggal selesai tidak valid"); return false; }
    return true
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!selectedMentor) {
        setError("Mentor yang dipilih tidak valid.")
        setLoading(false)
        return
      }
      ...
      <div className="p-5 lg:p-6 max-w-[1200px] mx-auto">
        <div className="mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl lg:text-3xl font-bold bg-gradient-to-r from-slate-800 via-emerald-800 to-teal-800 bg-clip-text text-transparent">Tambah Peserta Baru</h1>
              <p className="text-xs text-slate-500 flex items-center gap-1.5">
                <span className="w-1 h-1 bg-emerald-500 rounded-full"></span> Semua field wajib diisi (*)
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="space-y-5">
                <div className="flex items-center gap-2 mb-1">
                  <div className="p-1 bg-blue-100 rounded-lg"><Mail size={12} className="text-blue-600" /></div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input name="email" type="email" placeholder="contoh@email.com" value={form.email} onChange={handleChange} onBlur={() => handleBlur('email')} disabled={loading} className={`w-full pl-9 pr-10 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 transition ${getFieldError('email') ? 'border-red-300 focus:border-red-400 focus:ring-red-500/30 bg-red-50/30' : form.email && isEmailValid() ? 'border-emerald-300 focus:border-emerald-400 focus:ring-emerald-500/30' : 'border-slate-200 focus:border-emerald-400 focus:ring-emerald-500/30'}`} />
                    {form.email && isEmailValid() && !getFieldError('email') && <CheckCircle size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-emerald-500" />}
                  </div>
                  {getFieldError('email') && <p className="text-[10px] text-red-500 mt-1 flex items-center gap-1"><AlertTriangle size={10} /> {getFieldError('email')}</p>}
                </div>
                <div>
                  ...
                  {form.password && (
                    <div className="mt-1.5 flex items-center gap-2">
                      <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                        <div className={`h-full transition-all duration-300 ${passwordStrength.level === 3 ? 'w-full bg-emerald-500' : passwordStrength.level === 2 ? 'w-2/3 bg-amber-500' : 'w-1/3 bg-red-500'}`} />
                      </div>
                      <span className={`text-[9px] px-2 py-0.5 rounded-full font-medium ${passwordStrength.color}`}>{passwordStrength.text}</span>
                    </div>
                  )}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <select name="id_divisi" value={form.id_divisi} onChange={handleChange} onBlur={() => handleBlur('id_divisi')} disabled={loading || divisiLoading} className={`w-full pl-9 pr-10 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 transition appearance-none bg-white ${getFieldError('id_divisi') ? 'border-red-300 focus:border-red-400 focus:ring-red-500/30 bg-red-50/30' : form.id_divisi && isDivisiValid() ? 'border-emerald-300 focus:border-emerald-400 focus:ring-emerald-500/30' : 'border-slate-200 focus:border-emerald-400 focus:ring-emerald-500/30'}`}>
                      <option value="">Pilih Divisi</option>
                      {divisiLoading ? <option value="" disabled>Memuat divisi...</option> : divisiList.length === 0 ? <option value="" disabled>Tidak ada divisi tersedia</option> : divisiList.map((divisi) => (<option key={divisi.id_divisi || divisi.id} value={divisi.id_divisi || divisi.id}>{divisi.nama_divisi || divisi.nama}</option>))}
                    </select>
                    ...
                    {form.id_divisi && isDivisiValid() && !getFieldError('id_divisi') && <CheckCircle size={14} className="absolute right-8 top-1/2 -translate-y-1/2 text-emerald-500" />}
                  </div>
                  {getFieldError('id_divisi') && <p className="text-[10px] text-red-500 mt-1 flex items-center gap-1"><AlertTriangle size={10} /> {getFieldError('id_divisi')}</p>}
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1.5">Mentor Pembimbing <span className="text-red-500">*</span></label>
                  <div className="relative">
                    ...
                    <select name="id_mentor" value={form.id_mentor} onChange={handleChange} onBlur={() => handleBlur('id_mentor')} disabled={loading || mentorLoading || !form.id_divisi} className={`w-full pl-9 pr-10 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 transition appearance-none bg-white ${getFieldError('id_mentor') ? 'border-red-300 focus:border-red-400 focus:ring-red-500/30 bg-red-50/30' : form.id_mentor && isMentorValid() ? 'border-emerald-300 focus:border-emerald-400 focus:ring-emerald-500/30' : 'border-slate-200 focus:border-emerald-400 focus:ring-emerald-500/30'}`}>
                      <option value="">{!form.id_divisi ? "Pilih divisi terlebih dahulu" : mentorLoading ? "Memuat mentor..." : filteredMentors.length === 0 ? "Tidak ada mentor di divisi ini" : "Pilih Mentor"}</option>
                      {filteredMentors.map((mentor) => (<option key={mentor.id} value={mentor.id}>{mentor.name}</option>))}
                    </select>
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none"><svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></div>
                    {form.id_mentor && isMentorValid() && !getFieldError('id_mentor') && <CheckCircle size={14} className="absolute right-8 top-1/2 -translate-y-1/2 text-emerald-500" />}
                  </div>
                  {getFieldError('id_mentor') && <p className="text-[10px] text-red-500 mt-1 flex items-center gap-1"><AlertTriangle size={10} /> {getFieldError('id_mentor')}</p>}
                  {form.id_divisi && filteredMentors.length === 0 && !mentorLoading && (
                    <p className="text-[10px] text-amber-600 mt-1 flex items-center gap-1"><AlertCircle size={10} /> Belum ada mentor di divisi ini. <button onClick={() => navigate("/admin/add-mentor")} className="text-emerald-600 hover:underline font-medium ml-1">Tambah mentor sekarang</button></p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-5">
                <div className="flex items-center gap-2 mb-1">
                  <div className="p-1 bg-emerald-100 rounded-lg"><User size={12} className="text-emerald-600" /></div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input name="nama" type="text" placeholder="Masukkan nama lengkap" value={form.nama} onChange={handleChange} onBlur={() => handleBlur('nama')} className={`w-full pl-9 pr-10 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 transition ${getFieldError('nama') ? 'border-red-300 focus:border-red-400 focus:ring-red-500/30 bg-red-50/30' : form.nama && isNameValid() ? 'border-emerald-300 focus:border-emerald-400 focus:ring-emerald-500/30' : 'border-slate-200 focus:border-emerald-400 focus:ring-emerald-500/30'}`} disabled={loading} />
                    {form.nama && isNameValid() && !getFieldError('nama') && <CheckCircle size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-emerald-500" />}
                  </div>
                  {getFieldError('nama') && <p className="text-[10px] text-red-500 mt-1 flex items-center gap-1"><AlertTriangle size={10} /> {getFieldError('nama')}</p>}
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1.5">Nomor Telepon / WhatsApp <span className="text-red-500">*</span></label>
                  <div className="relative">
                    ...
                    <input name="no_telepon" type="tel" placeholder="81234567890" value={form.no_telepon} onChange={handleChange} onBlur={() => handleBlur('no_telepon')} className={`w-full pl-9 pr-10 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 transition ${getFieldError('no_telepon') ? 'border-red-300 focus:border-red-400 focus:ring-red-500/30 bg-red-50/30' : form.no_telepon && isPhoneValid() ? 'border-emerald-300 focus:border-emerald-400 focus:ring-emerald-500/30' : 'border-slate-200 focus:border-emerald-400 focus:ring-emerald-500/30'}`} disabled={loading} inputMode="numeric" />
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input name="asal_kampus" type="text" placeholder="Nama Universitas" value={form.asal_kampus} onChange={handleChange} onBlur={() => handleBlur('asal_kampus')} className={`w-full pl-9 pr-10 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 transition ${getFieldError('asal_kampus') ? 'border-red-300 focus:border-red-400 focus:ring-red-500/30 bg-red-50/30' : form.asal_kampus && isKampusValid() ? 'border-emerald-300 focus:border-emerald-400 focus:ring-emerald-500/30' : 'border-slate-200 focus:border-emerald-400 focus:ring-emerald-500/30'}`} disabled={loading} />
                    {form.asal_kampus && isKampusValid() && !getFieldError('asal_kampus') && <CheckCircle size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-emerald-500" />}
                  </div>
                  {getFieldError('asal_kampus') && <p className="text-[10px] text-red-500 mt-1 flex items-center gap-1"><AlertTriangle size={10} /> {getFieldError('asal_kampus')}</p>}
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1.5">Program Studi <span className="text-red-500">*</span></label>
                  <div className="relative">
                    ...
                    <input name="prodi" type="text" placeholder="Program Studi" value={form.prodi} onChange={handleChange} onBlur={() => handleBlur('prodi')} className={`w-full pl-9 pr-10 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 transition ${getFieldError('prodi') ? 'border-red-300 focus:border-red-400 focus:ring-red-500/30 bg-red-50/30' : form.prodi && isProdiValid() ? 'border-emerald-300 focus:border-emerald-400 focus:ring-emerald-500/30' : 'border-slate-200 focus:border-emerald-400 focus:ring-emerald-500/30'}`} disabled={loading} />
                    {form.prodi && isProdiValid() && !getFieldError('prodi') && <CheckCircle size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-emerald-500" />}
                  </div>
                  {getFieldError('prodi') && <p className="text-[10px] text-red-500 mt-1 flex items-center gap-1"><AlertTriangle size={10} /> {getFieldError('prodi')}</p>}
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-slate-200">
              <div className="flex items-center gap-2 mb-4">
                <div className="p-1.5 bg-gradient-to-br from-amber-500 to-orange-500 rounded-lg shadow-sm"><Briefcase size={14} className="text-white" /></div>
                ...
                {duration && duration > 0 && (
                  <div className="ml-auto flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50 rounded-full border border-emerald-200">
                    ...
                    <span className="text-[10px] font-semibold text-emerald-700">Durasi: {duration} Hari</span>
                  </div>
                )}
              </div>
              ...
              {form.tanggal_mulai && form.tanggal_selesai && isTanggalSelesaiValid() && (
                <div className="mt-4 p-3 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-lg border border-emerald-100">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-slate-600">{formatDate(form.tanggal_mulai)} - {formatDate(form.tanggal_selesai)}</span>
            ...
            <div className="mt-6 pt-4 border-t border-slate-100">
              <div className="flex items-center justify-between text-xs text-slate-500 mb-2">
                <span className="flex items-center gap-1"><Award size={12} /> Progress pengisian form</span>
                <span className="font-medium text-emerald-600">{completedCount}/{totalRequired} field wajib terisi</span>
              </div>
          ...
          <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
            <button type="button" onClick={() => navigate("/admin/users")} className="px-5 py-2 border border-slate-200 rounded-lg text-slate-600 text-sm font-medium hover:bg-white hover:border-slate-300 transition-all duration-200" disabled={loading}>Batal</button>
                ...
                <div className="mx-6 p-5 bg-gradient-to-br from-slate-50 to-white rounded-xl border border-slate-200 shadow-md">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div><h4 className="font-bold text-slate-800 text-lg">{successData.nama}</h4></div>
                    </div>
                    <div className="px-2 py-1 bg-emerald-100 rounded-lg"><p className="text-[9px] font-semibold text-emerald-600">PESERTA</p></div>
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3 p-2 bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg border border-amber-100">
                      <div className="w-7 h-7 bg-amber-100 rounded-lg flex items-center justify-center"><Briefcase size={13} className="text-amber-600" /></div>
                      <div className="flex-1"><p className="text-[9px] text-slate-400">Periode Magang</p><p className="text-xs font-medium text-slate-700">{successData.tanggal_mulai} - {successData.tanggal_selesai}</p></div>
                      <div className="px-1.5 py-0.5 bg-emerald-100 rounded"><p className="text-[9px] font-semibold text-emerald-600 flex items-center gap-0.5"><CalendarRange size={8} /> {successData.durasi} Hari</p></div>
                    </div>
                  </div>
                </div>
