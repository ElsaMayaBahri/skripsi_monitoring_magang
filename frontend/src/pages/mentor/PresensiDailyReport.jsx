...
function PresensiDailyReport() {
  ...
  const fetchDailyReports = async () => {
    ...
    try {
      ...
      alert("Gagal memuat data laporan. Silakan coba lagi nanti.");
      setAllReports([]);
      setFilteredReports([]);
      setSummary({ total: 0, sudahMengisi: 0, belumMengisi: 0, persentase: 0 });
    } finally {
  ...
  const exportToExcel = async () => {
    try {
      const response = await exportDailyReport(selectedDate);
      if (response && response.success && response.data) {
        const data = response.data;
      ...
      alert("Gagal mengexport data: " + (error.response?.data?.message || "Terjadi kesalahan"));
    }
  };
