...
function EditMateri() {
  ...
  const validateForm = () => {
    ...
    if (formDataState.tipe_materi === "link") {
      if (!formDataState.link || formDataState.link.trim() === "") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      submitData.append("_method", "PUT"); // Penting untuk Laravel
      
      // PERBAIKAN: Kirim file jika ada file baru
      if (formDataState.file_materi) {
        console.log("Uploading new file:", formDataState.file_materi.name);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 via-white to-teal-100/20">
      ...
      <div className="relative p-6 lg:p-8 max-w-[1000px] mx-auto">
        ...
        <form onSubmit={handleSubmit} className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
              ...
              <div>
                ...
                <div className={`border-2 border-dashed rounded-xl p-6 text-center transition-all duration-200 ${
                  ...
                  <input
                    ...
                    className="hidden"
                    id="file_upload"
                    accept={formDataState.tipe_materi === "video" ? "video/mp4,video/quicktime,video/x-msvideo,video/webm" : ".pdf,.doc,.docx,.ppt,.pptx"}
                  />
                  {(formDataState.file_materi || (oldFile && !removeFile)) ? (
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button
                          type="button"
                          onClick={resetFile}
                          className="p-1.5 rounded-xl bg-rose-50 text-rose-500 hover:bg-rose-100 transition-all duration-200"
                        >
                          <X size={14} />
                        </button>
                      </div>
                      {formDataState.tipe_materi === "video" && filePreview && formDataState.file_materi && (
                    ...
                    <label htmlFor="file_upload" className="cursor-pointer block">
                      <div className="w-14 h-14 mx-auto rounded-2xl bg-gradient-to-br from-teal-100 to-blue-100 flex items-center justify-center mb-3">
                        {formDataState.tipe_materi === "video" ? (
                ...
                {errors.file_materi && <p className="text-xs text-red-500 mt-1">{errors.file_materi}</p>}
              </div>
            )}
          ...
          <div className="px-6 py-4 bg-gradient-to-r from-slate-50 to-white border-t border-slate-100 flex justify-end gap-3">
