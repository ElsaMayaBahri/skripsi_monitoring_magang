...
function LaporanPresensi() {
  ...
  // Hitung rentang hari - DENGAN VALIDASI DAN TANPA Math.abs()
  const getDateRangeDays = () => {
    let start = startDate
    let end = endDate
    
    // Jika hanya startDate yang diisi, anggap single date
    if (startDate && !endDate) {
      start = startDate
      end = startDate
    }
    // Jika hanya endDate yang diisi, anggap single date
    if (!startDate && endDate) {
      start = endDate
      end = endDate
    }
    ...
    // VALIDASI: jika startDate > endDate, return 0
    if (startObj > endObj) {
      return 0
    }
    
    // HITUNG SELISIH TANPA Math.abs() - pastikan start <= end
  ...
  const fetchPresensi = async () => {
    ...
    try {
      ...
      // LOGIC CERDAS UNTUK FILTER TANGGAL
      ...
      if (startDate || endDate) {
        ...
        // Jika hanya startDate yang diisi -> single date
        if (startDate && !endDate) {
          start = new Date(startDate)
          end = new Date(startDate)
          end.setHours(23, 59, 59)
        }
        // Jika hanya endDate yang diisi -> single date
        else if (!startDate && endDate) {
          start = new Date(endDate)
          end = new Date(endDate)
          end.setHours(23, 59, 59)
        }
        // Jika kedua diisi -> range date (dengan validasi start <= end)
        else if (startDate && endDate) {
          const startObj = new Date(startDate)
          const endObj = new Date(endDate)
          
          // Jika start > end, tukar
          if (startObj > endObj) {
            setToast({ 
              show: true, 
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      if (startDate && !endDate) {
        ...
        periodText = `${startDate}_sampai_${endDate}`
      }
  ...
  const handleExportPDF = () => {
    ...
    if (filteredData.length > MAX_DATA_FOR_PDF) {
      setToast({ 
        show: true, 
        message: `Data terlalu banyak (${filteredData.length} peserta). Disarankan menggunakan format Excel.`, 
        type: "warning" 
      })
      setShowExportDropdown(false)
      return
    ...
    if (rangeDays > THREE_MONTHS) {
      setToast({ 
        show: true, 
        message: `Rentang waktu ${rangeDays} hari melebihi batas maksimal PDF (3 bulan). Silakan gunakan format Excel.`, 
        type: "warning" 
      })
      setShowExportDropdown(false)
      return
  ...
  const executePDFExport = () => {
    ...
    try {
      ...
      let startDateFormatted = "Semua"
      let endDateFormatted = "Semua"
      let periodeText = ""
      ...
      if (startDate && !endDate) {
        ...
        startDateFormatted = formatDatePDF(startDate)
        endDateFormatted = formatDatePDF(endDate)
        periodeText = `Periode: ${startDateFormatted} s/d ${endDateFormatted}`
      } else {
      ...
      element.innerHTML = `
        ...
        <div style="text-align: center; margin-bottom: 25px;">
          ...
          <p style="color: #64748b; margin: 8px 0 0 0; font-size: 12px;">${periodeText}</p>
