...
import { getAllQuiz, deleteQuiz, importQuiz } from "../../api/coo/quizService";
...
function Quiz() {
  ...
  const getQuizStatus = (q) => {
    if (q.is_belum_mulai) return "akan_datang";
    if (q.is_selesai) return "selesai";
    if (q.is_aktif) return "aktif";

    const status = String(q.status || "").toLowerCase();
    if (status === "aktif" || status === "active") return "aktif";
    if (status === "akan_datang") return "akan_datang";
    ...
    try {
      ...
      const transformedData = quizData.map((q) => {
        const questions = Array.isArray(q.questions)
          ? q.questions
          : Array.isArray(q.soal)
            ? q.soal
            : [];

        const totalSoal = toNumber(
          q.total_soal ??
            q.total_questions ??
            q.jumlah_soal ??
            questions.length,
        ...
        return {
          id: q.id_kuis ?? q.id,
          id_kuis: q.id_kuis ?? q.id,
          judul: q.judul_kuis ?? q.judul ?? q.title ?? "Tanpa Judul",
          deskripsi: q.deskripsi ?? "",
          divisi: q.divisi ?? "Umum",
          durasi: toNumber(q.durasi ?? q.duration, 30),
          peserta: toNumber(q.peserta ?? q.participants, 0),
          ...
          passing: toNumber(q.passing ?? q.passing_score, 75),
          level: toNumber(q.level, 1),
        };
      });
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (response.success) {
        const importedCount =
          response.data?.imported || response.data?.length || 0;
  ...
  const getLevelBadge = (level) => {
    const levelConfig = {
      1: {
        ...
        icon: <Star size={12} className="text-emerald-500" />,
        label: "Level 1 - Kuis Pertama",
      },
      2: {
        bg: "bg-blue-100",
        text: "text-blue-700",
        ...
        label: "Level 2 - Kuis Kedua",
      },
      3: {
        bg: "bg-purple-100",
        text: "text-purple-700",
        border: "border-purple-200",
        icon: <Target size={12} className="text-purple-500" />,
        label: "Level 3 - Kuis Ketiga",
      },
    };
    ...
    return (
      <div
        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full ${config.bg} ${config.text} border ${config.border}`}
      >
        {config.icon}
  ...
  const filtered = quiz.filter((q) => {
    ...
    const matchesDivisi =
      selectedDivisis.length === 0 || selectedDivisis.includes(q.divisi);
    const matchesStatus =
      selectedStatus === "all" || q.status === selectedStatus;
    const matchesLevel =
      selectedLevel === "all" || q.level === parseInt(selectedLevel);
  ...
  const sortedLevels = [1, 2, 3].filter(
    (level) => groupedQuiz[level] && groupedQuiz[level].length > 0,
  );
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-lg p-5 mb-6 animate-in fade-in duration-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div>
                ...
                <select
                  value={
                    selectedDivisis.length === 1 ? selectedDivisis[0] : "all"
                  }
                  onChange={(e) => {
              ...
              <div>
                ...
                >
                  ...
                  <option value="2">Level 2 - Kuis Kedua</option>
                  <option value="3">Level 3 - Kuis Ketiga</option>
                </select>
              </div>
        ...
        <div className="space-y-5">
          {sortedLevels.map((level) => (
            ...
            >
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center bg-gradient-to-r ${getLevelColor(level)} shadow-md`}
                  >
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 mt-0.5">
                      {level === 1 &&
                        "Kuis pertama yang harus diselesaikan peserta terlebih dahulu"}
                      {level === 2 &&
                        "Kuis kedua akan terbuka setelah peserta lulus level 1"}
                      {level === 3 &&
                        "Kuis ketiga akan terbuka setelah peserta lulus level 2"}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-1 text-xs text-slate-500">
                    <Users size={14} />
                    <span>
                      {groupedQuiz[level].reduce(
                        (sum, q) => sum + (q.peserta || 0),
                        0,
                      )}{" "}
                      peserta
                    </span>
          ...
          {sortedLevels.length === 0 && (
            <div className="bg-white rounded-2xl border border-slate-200 shadow-lg p-12 text-center">
              ...
              <button
                onClick={resetFilters}
                className="mt-4 flex items-center gap-2 px-5 py-2.5 bg-slate-100 text-slate-700 rounded-xl font-medium hover:bg-slate-200 transition-all"
              >
                Reset Filter
              </button>
            </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 leading-relaxed">
                Kuis dikelompokkan berdasarkan{" "}
                <strong className="text-indigo-600">Level 1 sampai Level 3</strong>.
                Peserta harus menyelesaikan dan{" "}
                <strong className="text-emerald-600">LULUS</strong> (nilai ≥
                passing grade) pada level sebelumnya untuk dapat mengakses kuis
                di level berikutnya. Gunakan ikon{" "}
                <BarChart3 size={12} className="inline text-purple-600" /> untuk
                melihat hasil dan progress peserta per level.
              </p>
            </div>
          </div>
        </div>
