...
/**
 * Update sertifikat template by ID (for edit)
 * @param {number} id - Template ID
 * @param {FormData} formData - Form data with file and template info
 */
export const updateSertifikatTemplate = async (id, formData) => {
  try {
    // Laravel menggunakan POST dengan _method PUT untuk update file
    const response = await axiosInstance.post(`/sertifikat/templates/${id}`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    });
    console.log('✏️ Update Sertifikat Template Response:', response.data);
    return response.data;
  } catch (error) {
    console.error("Error updating sertifikat template:", error);
    throw error;
  }
};
...
export const deleteSertifikatTemplate = async (id) => {
  try {
    ...
    console.log('🗑️ Delete Sertifikat Template Response:', response.data);
    return response.data;
  } catch (error) {
...
export const toggleTemplateStatus = async (id, isActive) => {
  try {
    const response = await axiosInstance.put(`/sertifikat/templates/${id}/toggle-status`, {
      is_active: isActive
    });
    console.log('🔄 Toggle Template Status Response:', response.data);
    return response.data;
  } catch (error) {
